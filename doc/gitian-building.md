...
*Setup instructions for a Gitian build of Graincoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Graincoin
...
- [Building Graincoin](#building-graincoin)
...
Get the [Debian 8.x net installer](http://cdimage.debian.org/debian-cd/current/amd64/iso-cd/debian-8.7.1-amd64-netinst.iso) (a more recent minor version should also work, see also [Debian Network installation](https://www.debian.org/CD/netinst/)).
This DVD image can be validated using a SHA256 hashing tool, for example on
Unixy OSes by entering the following in a terminal:

    echo "749a15b7690769dd99e85d6104182a03d370b04b36106ec7cfaf6a551aa89fb4  debian-8.7.1-amd64-netinst.iso" | sha256sum -c
    # (must return OK)
...
Clone the git repositories for graincoin and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/graincoin-project/graincoin
```
...
in the graincoin repository under 'Fetch and build inputs' to install sources which require
...
To build Graincoin (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the graincoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/graincoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/graincoin/graincoin
...
For example:
```bash
URL=https://github.com/grain-crypto/grain.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit graincoin=${COMMIT} --url graincoin=${URL} ../graincoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit graincoin=${COMMIT} --url graincoin=${URL} ../graincoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit graincoin=${COMMIT} --url graincoin=${URL} ../graincoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the graincoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../graincoin/contrib/gitian-descriptors/*|sort|uniq )
...
GRAPATH=/some/root/path/graincoin.git
SIGPATH=/some/root/path/graincoin-detached-sigs.git

./bin/gbuild --url graincoin=${GRAPATH},signature=${SIGPATH} ../graincoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/graincoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/graincoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/graincoin-osx-build.assert
```
...
[graincoin-project/gitian.sigs](https://github.com/graincoin-project/gitian.sigs) repository, or if that's not possible create a pull
request. You can also mail the files to Getreidemakler (getreidemakler@blkid.net) and he will commit them.
